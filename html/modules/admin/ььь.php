...
foreach ($checks as &$check) {
    ...
    if (!empty($check['items'])) {
        ...
        // Дополнительная проверка: возможно, товар хранится как объект, а не массив
        else if (is_object($items) || (is_array($items) && isset($items['name']))) {
            // Один товар в виде объекта
            $name = $items['name'] ?? $items['title'] ?? 'Без названия';
