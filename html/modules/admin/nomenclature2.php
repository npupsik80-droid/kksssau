...
if ($search) {
    $where = "WHERE n.name LIKE ? OR n.barcode LIKE ?";
    $params = ["%{$search}%", "%{$search}%"];
}

// Вариант 1: Передаём параметры как целые числа
$query = "
...
// LIMIT и OFFSET передаём как целые числа
...
$stmt = $pdo->prepare("SELECT COUNT(*) as total FROM nomenclatures n {$where}");
$stmt->execute($params);
...
function updateNomenclature() {
    ...
    // Проверяем уникальность штрих-кода (кроме текущей записи)
    $stmt = $pdo->prepare("SELECT id FROM nomenclatures WHERE barcode = ? AND id != ?");
    ...
    <div class="container">
        ...
        <div class="stats-grid">
            ...
            <div class="stat-card fade-in-up" style="animation-delay: 0.3s;">
                ...
                <h3>Процент использования</h3>
                <div class="stat-value">
                    <?php 
                        $percentage = $total > 0 ? round(($used_count / $total) * 100) : 0;
                        echo $percentage; 
                    ?>%
                </div>
                <div style="color: #64748b; font-size: 14px;">От общего количества</div>
            </div>
        </div>
