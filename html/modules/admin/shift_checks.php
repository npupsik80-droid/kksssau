...
// Устанавливаем режим ошибок PDO на исключения (если ещё не сделано)
...
try {
    ...
    if (!$shift) {
        $_SESSION['error'] = 'Смена не найдена';
        header('Location: monitoring.php');
        exit();
    }
    ...
    // Статистика по чекам (используем COALESCE для защиты от NULL)
    ...
    error_log("Ошибка БД в shift_checks.php: " . $e->getMessage());
    $_SESSION['error'] = 'Ошибка базы данных. Подробности в логах.';
    header('Location: monitoring.php');
    exit();
}

// ... (дальнейшая обработка чеков и HTML без изменений)
...
<head>
    ...
    <title>Чеки смены #<?php echo $shift['kkm_shift_number']; ?> - RunaRMK</title>
    <style>
        :root {
            --primary: #4361ee;
            --secondary: #2ecc71;
...
<body>
    ...
    <div class="header">
        <h1>
            ...
            Чеки смены #<?php echo $shift['kkm_shift_number']; ?>
        </h1>
                ...
                <div class="checks-table desktop-view">
                    ...
                    <div class="table-row">
                        ...
                        <div class="table-cell action-buttons">
                            ...
                            <button class="btn btn-secondary btn-sm" onclick="exportCheck(<?php echo $check['id']; ?>)">
    ...
    <script>
        ...
        function exportCheck(checkId) {
            window.open(`export_check.php?id=${checkId}`, '_blank');
        }
        
        // Экспорт всех чеков
        function exportChecks(format) {
            const shiftId = <?php echo $shift_id; ?>;
            window.open(`export_checks.php?shift_id=${shiftId}&format=${format}`, '_blank');
        }
