...
internal class GateUCS : UnitPort
{
	...
	[DllImport("ucs_ms.dll")]
	private static extern nint eftp_create(nint szConfigPath);

	[DllImport("ucs_ms.dll")]
	private static extern int eftp_do(nint pvSelf, nint pchInBuffer, nint pchOutBuffer, nint pfIdle, nint pvData);

	[DllImport("ucs_ms.dll")]
	private static extern void eftp_destroy(nint pvSelf);

	public GateUCS(Global.DeviceSettings SettDr, int NumUnit)
		: base(SettDr, NumUnit)
	{
	...
	public async Task<UCSResponce> RunCommand(string In, string Out, int TimeOut = 30000, params string[] Pars)
	{
		...
		while (true)
		{
			...
			if (SetPort.TypeConnect != SetPorts.enTypeConnect.Software)
			{
			...
			bool IsStartOperayion = false;
			UCSResponce uCSResponce;
			while (true)
			{
				uCSResponce = ((SetPort.TypeConnect != SetPorts.enTypeConnect.Software) ? (await GetFrame(TimeOut, WriteError)) : DoDll(In, Pars, TimeOut, ref IsStartOperayion));
				if (uCSResponce == null)
				{
				...
				if (SetPort.TypeConnect == SetPorts.enTypeConnect.Software)
				{
					while (uCSResponce.Rez != 9)
					{
						uCSResponce = DoDll(In, Pars, TimeOut, ref IsStartOperayion);
					}
				}
				if (OpenSerial)
				{
	...
	public override async Task<bool> PortOpenAsync()
	{
		...
		if (SetPort.TypeConnect == SetPorts.enTypeConnect.Software)
		{
			...
			Monitor.Enter(Lock_Dll);
			string text = Path.Combine(DirectoryDll, "BIN", "ucs_ms.dll");
			ExecuteDll.RunDLL(ref Dllptr, text, ref DllIsLosd);
			PortLogs.Append("+ dll '" + text + "' загружена.");
			byte[] bytes = Encoding.ASCII.GetBytes("micromgl.cfg\0");
			nint num = Marshal.AllocHGlobal(512);
			Marshal.Copy(bytes, 0, num, bytes.Length);
			pvSelf = eftp_create(num);
			Marshal.FreeHGlobal(num);
			PortLogs.Append("< Объект получен: '" + (IntPtr)pvSelf + "'");
	...
	public override async Task<bool> PortCloseAsync()
	{
		...
		if (SetPort.TypeConnect == SetPorts.enTypeConnect.Software)
		{
			...
			eftp_destroy(pvSelf);
			PortLogs.Append("> Объект удален.");
			pvSelf = 0;
			ExecuteDll.StopDll(ref Dllptr, ref DllIsLosd);
			PortLogs.Append("- dll остановлена.");
			SetPort.PortOpen = false;
			Monitor.Exit(Lock_Dll);
			return true;
		}
		return await base.PortCloseAsync();
	}

	public UCSResponce DoDll(string In, string[] Pars, int TimeOut, ref bool IsStartOperayion)
	{
		UCSResponce uCSResponce = new UCSResponce();
		UCSStryctResponce uCSStryctResponce = ParTypes[In];
		int num = 0;
		int num2 = 0;
		string[] array = Pars;
		...
		text2 += c;
		text2 += c2;
		text2 += TerminalID.PadLeft(10, '0');
		text2 += num.ToString("X2");
		int num4 = 0;
		array = Pars;
		for (int i = 0; i < array.Length; i++)
		{
			string text3 = array[i];
			int num5 = uCSStryctResponce.Params[num4];
			if (num5 != -1 && num5 != 0 && num5 > text3.Length)
			{
				text3 = text3.PadLeft(num5, '0');
			}
			else if (num5 != -1 && num5 != 0 && num5 < text3.Length)
			{
		...
		if (!IsStartOperayion)
		{
			num6 = Marshal.AllocHGlobal(512);
			byte[] array4 = new byte[512];
			byte[] bytes = Win1251.GetBytes(text2 + "\0");
			Buffer.BlockCopy(bytes, 0, array4, 0, Math.Min(512, bytes.Length));
			Marshal.Copy(array4, 0, num6, 512);
		}
		else
		{
			text2 = "<null>";
		}
		nint num7 = Marshal.AllocHGlobal(512);
		uCSResponce.Rez = eftp_do(pvSelf, num6, num7, 0, 0);
		byte[] array5 = new byte[512];
		Marshal.Copy(num7, array5, 0, array5.Length);
		string text4 = Win1251.GetString(array5);
		if (!IsStartOperayion)
		{
			Marshal.FreeHGlobal(num6);
			IsStartOperayion = true;
		}
		Marshal.FreeHGlobal(num7);
		try
		{
			uCSResponce.Command = text4[0] + "-" + text4[1];
			if (int.Parse(text4.Substring(2, 10)).ToString("D") != TerminalID)
			{
				Error = Error + ((Error == "") ? "" : "; ") + "Ошибка приема кадра сообщения - не правильный номер терминала";
				PortLogs.Append(Error);
				base.PortReadTimeout = 1000;
				PortReadByteAsync().Wait();
				return null;
			}
		}
		catch
		...
		uCSResponce.Length = int.Parse(text4.Substring(12, 2), NumberStyles.HexNumber);
		int num8 = text4.IndexOf('\0');
		num8 = ((num8 == -1 || num8 < 14) ? uCSResponce.Length : Math.Min(uCSResponce.Length, num8 - 14));
		string text5 = text4.Substring(14, num8);
		string command = uCSResponce.Command;
		if (!ParTypes.ContainsKey(command))
		{
			uCSResponce.Data.Add(text5);
		}
		else
		{
			uCSStryctResponce = (uCSResponce.Stryct = ParTypes[command]);
			int num9 = 0;
			array3 = uCSStryctResponce.Params;
			foreach (int num10 in array3)
			{
				string text6 = "";
				int num11 = ((num10 == -1) ? 10000 : num10);
				for (int j = 0; j < num11; j++)
				{
					if (text5.Length > num9)
					{
						if (text5.Substring(num9, 1) == "\u001b")
						{
							num9++;
							break;
						}
						text6 += text5.Substring(num9, 1);
						num9++;
					}
				}
				uCSResponce.Data.Add(text6);
			}
			if (uCSStryctResponce.LastLineFlag)
			{
				uCSResponce.LastLineFlag = uCSResponce.Data[0] == "1";
			}
		}
		if (uCSResponce.Command == "0-0")
		{
			return null;
		}
		return uCSResponce;
